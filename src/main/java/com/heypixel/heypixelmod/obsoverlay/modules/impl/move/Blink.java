...
public class Blink extends Module {
    ...
    private boolean isPlayerNear(double distance) {
        long players = mc.level.players().stream().filter(player -> {
            if (player == mc.player) {
                ...
                return RotationUtils.getDistance(this.fakePlayer, player) < distance;
            }
        }).count();
        return players > 0L;
    }
    ...
    private boolean isPlayerInDanger() {
        return this.isTNTNear(this.tntDistance.getCurrentValue())
                || this.isPlayerNear(this.playerDistance.getCurrentValue())
                || this.isArrowNear(this.projectilesExpands.getCurrentValue());
    }
